 ...
 * 	File:			myNode.h
 * 	Author:			Raul Zuniga
 * 	Description:	Provides a template class for a node in a binary tree and  functions for manipulating binary trees.
 * 					The template parameter is the type of data in each node.
 *
 * 	TYPEDEFS for the myNode<Item> template class:
 * 	Each node of the tree contains a piece of data and pointers to its children.
 * 	The type of the data (myNode<Item>::value_type) is the Item type from the template parameter.
 * 	The type may be any of  the C++ built-in types(int, char, etc.), or a class with a default  constructor, and an
 * 	assignment operator.
 *
 * 	CONSTRUCTOR for myNode<Item> class:
 * 	myNode( const Item& init_data = Item(), myNode* init_left = NULL, myNode* init_right = NULL );
 * 	Postcondition:  The new node has its data equal to init_data, and its child pointers equal to init_left and init_right.
 *
 * 	MEMBER FUNCTIONS for the myNode<Item> class:
 * 	const Item&data() const		  			<----- const version
 * 	Item& data()							<----- non-const version
 * 	Postcondition: The return value is a reference to the data from this myNode.
 *
 ...
 * 	Postcondition: The return value is a pointer to the left or right child(which will be NULL if there is no child)
 *
 ...
 * 	Postcondition: The myNode now contains the specified new link to a child.
 *
 * 	bool is_leaf()
 * 	Postcondition: The return value is true if the node is a leaf; otherwise the return value is false.
 *
 * 	NONMEMBER FUNCTIONS to manipulate binary tree nodes:
 *
 * 	template <class Item>
 * 	void tree_clear(myNode<Item>*& root_ptr)
 * 	Precondition: root_ptr is the root pointer of a binary tree(whic may be NULL for the empty tree).
 ...
 * 	Postcondition: A copy of the binary tree has been made, and the return value is a pointer to the root of this copy.
 *
 * 	DYNAMIC MEMORY usage:
 * 	If there is insufficient dynamic memory, then the following functions throw bad_alloc:  tree _copy.
***********************************************************************************************************************/
