...
void setColor(ColorName name) {
    switch(name) {
        ...
        case MAGENTA:
            ...
            break;
        case BROWN:
            glColor3d(0.36, 0.16, 0.1);
            break;
        default:
            cerr << "Trying to set invalid color. Color unchanged" << endl;
    }
}
...
void drawFilledOval(int x, int y, int width, int height, int rotateAngle) {
    double angle = 0.0, rotateAngleRad = PI*rotateAngle/180.0;
...
void drawArc(int x, int y, int radius, int startAngle, int endAngle) {
    ...
    glBegin(GL_LINE_STRIP);
    for (double angle = startRadians; fabs(angle-endRadians) >= ANGLE_STEP ; angle = wrapAngle(angle + ANGLE_STEP)) {
...
void GraphicsTimer::start() {
	lastElapsed = glutGet(GLUT_ELAPSED_TIME);
}

int GraphicsTimer::sinceStart() {
	return glutGet(GLUT_ELAPSED_TIME) - lastElapsed;
}

int GraphicsTimer::deltaT() {
